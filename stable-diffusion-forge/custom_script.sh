...
curl -fsS "https://github.com/hdae/danbooru-tagcomplete-extra/releases/download/2025-05-06/danbooru.csv" -o /workspace/extensions/tag-autocomplete/tags/danbooru.csv

# Add config files.
SDF_CONFIG="https://raw.githubusercontent.com/hdae/runpod-custom-scripts/refs/heads/main/stable-diffusion-forge"
curl -fsS "${SDF_CONFIG}/config.json" -o /workspace/config.json
curl -fsS "${SDF_CONFIG}/ui-config.json" -o /workspace/ui-config.json
